...
namespace TKBase.Framework.Fleck
{
    public class SocketWrapper : ISocket
    {
        ...
        public string RemoteIpAddress
        {
            get
            {
                return _socket.RemoteEndPoint is IPEndPoint endpoint ? endpoint.Address.ToString() : null;
            }
        }
        ...
        public Task<int> Receive(byte[] buffer, Action<int> callback, Action<Exception> error, int offset)
        {
            try
            {
                IAsyncResult begin(AsyncCallback cb, object s) => _stream.BeginRead(buffer, offset, buffer.Length, cb, s);
        ...
        {
            ISocket end(IAsyncResult r) => _tokenSource.Token.IsCancellationRequested ? null : new SocketWrapper(_socket.EndAccept(r));
        ...
        public Task Send(byte[] buffer, Action callback, Action<Exception> error)
        {
            ...
            try
            {
                IAsyncResult begin(AsyncCallback cb, object s) => _stream.BeginWrite(buffer, 0, buffer.Length, cb, s);
