...
namespace TKBase.DotNetty.Common.Utilities
{
    using System;
    ...
    public sealed class HashedWheelTimer : ITimer
    {
        ...
        sealed class Worker : IRunnable
        {
            ...
            void TransferTimeoutsToBuckets()
            {
                ...
                {
                    if (!this.owner.timeouts.TryDequeue(out HashedWheelTimeout timeout))
                    {
                        // all processed
                        break;
                    }
            ...
            void ProcessCancelledTasks()
            {
                while (true)
                {
                    if (!this.owner.cancelledTimeouts.TryDequeue(out HashedWheelTimeout timeout))
                    {
                        // all processed
                        break;
                    }
