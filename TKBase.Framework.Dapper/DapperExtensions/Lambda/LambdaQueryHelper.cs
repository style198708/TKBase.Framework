...
using TKBase.Framework.Dapper.Mapper;
using TKBase.Framework.Dapper.Sql;
using TKBase.Framework.Dapper.ValueObject;
...
namespace TKBase.Framework.Dapper.Lambda
{
    ...
    public class LambdaQueryHelper
    {
        ...
        //public List<LambdaQueryHelper> queryHelpers = new List<LambdaQueryHelper>();

        //public LambdaQueryHelper AddLambdaQueryHelpe<T>(IWhere<T> where, IOrderBy<T> orderBy) where T : class
        //{
        //    queryHelpers.Add(DapperExtension.Instance(this.DbType).LambdaQuery<T>(this.Connection, null, null).Where(where).OrderBy(orderBy));
        //    return this;
        //}
        //public LambdaQueryHelper AddLambdaQueryHelpe<T>(LambdaQueryHelper<T> helper) where T : class
        //{
        //    queryHelpers.Add(helper);
        //    return this;
        //}
        ///// <summary>
        ///// 多表 
        ///// </summary>
        //public Tuple<IEnumerable<T>, IEnumerator<T>> ToMutiPageList<T>(int pageIndex, int pageSize) where T : class
        //{
        //    string execSql = string.Empty;
        //    queryHelpers.ForEach(p =>
        //    {

        //        string sql = p.GetSqlString(pageIndex, pageSize);
        //        string sqlcount = p.GetCountSqlString();

        //        execSql += sql + " ; " + sqlcount;
        //        DynamicParameters dynamicParameters = new DynamicParameters();
        ...
        public T ToEntity<T>() where T : class
        {
            ...
            if (ConnObj.DbConnection != null)
            {
                return Connection.Query<T>(sql, dynamicParameters, Transaction, true, CommandTimeout, CommandType.Text).AsList<T>().FirstOrDefault();
            }
            else
            {
                using (IDbConnection conn = DBUtils.CreateDBConnection(ConnKey, out DataBaseType dbType))
