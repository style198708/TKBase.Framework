...
namespace TKBase.Framework.Elasticsearch
{
    public class ElasticsearchHelper
    {
        ...
        {
            ElasticsearchConfig config = Config.Bind<ElasticsearchConfig>("elasticsearch.json");
            if (Client == null)
            {
                ...
                connectionSettings.DefaultIndex("db_elog");
        ...
        {
            CreateIndex<T>();
            Client.Index<T>(entity, pd => pd.Index(IndexName).Type<T>());
            return true;
        }
        ...
        public static bool Update<T>(T entity) where T : ElasticsearchBase
        {
            ...
            DocumentPath<T> deletePath = new DocumentPath<T>(entity.Id);
        ...
        public static T GetByID<T>(string Id) where T : ElasticsearchBase
        {
            CreateIndex<T>();
            var response = Client.Get(new DocumentPath<T>(Id), pd => pd.Index(IndexName).Type<T>());
            return response.Source;
        ...
        /// <returns></returns>
        public static ISearchResponse<T> Query<T>(Func<SearchDescriptor<T>, ISearchRequest> request) where T : ElasticsearchBase
        {
            CreateIndex<T>();
            return Client.Search<T>(request);  
        }
        ...
        /// <returns></returns>
        public static ISearchResponse<T> Query<T>(ISearchRequest request) where T : ElasticsearchBase
        {
